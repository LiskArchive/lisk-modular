...
module.exports = class ChildProcessChannel extends BaseChannel {
	constructor(moduleAlias, events, actions, options={}){
		...
		const rpcSocketPath = `${homeDir}/.lisk-core/sockets/${this.moduleAlias}_rpc.sock`;
		...
		this.rpcServer.expose('invoke', (actionName, params, cb) => {
			this.invoke(actionName, params).then((result) => setImmediate(cb, null, result)).catch(error => setImmediate(cb, error));
		});
	...
	subscribe(eventName, cb) {
		...
		this.eventsMap[event.toEmitterName()] = cb;
	}

	publish(eventName, data) {
		const event = new Event(eventName, data, this.moduleAlias);

		process.send({eventName: event.toEmitterName(), eventData: event.data});
	}
	...
	async invoke(actionName, params) {
		...
		if (action.module === this.moduleAlias) {
			...
			return new Promise((resolve, reject) => {
				this.busRpcClient.call('invoke', action.module, action.name, params, (err, data) => {
					if(err) {
